...
function main()
  ...
  sampRegisterChatCommand(
    ...
    function()
      lua_thread.create(
        function()
          updateMenu()
          submenus_show(mod_submenus_sa, "{348cb2}GMAP v." .. thisScript().version, "Выбрать", "Закрыть", "Назад")
        end
      )
    end
  )
  ...
  local timeout_count = 0
	...
	if settings.welcome.show then
		...
		sampAddChatMessage(
			"GMAP v" ..
			thisScript().version ..
			" к вашим услугам. Подробная информация: /gmap. Приятной игры, " .. licensenick .. ".",
			0x7ef3fa
		)
  ...
  while true do
    ...
    while connected do
      ...
      if sampGetCurrentServerAddress() ~= "95.181.158.64" and sampGetCurrentServerAddress() ~= "95.181.158.78" then
        do_not_reload = true
...
function dn(nam)
  ...
  if not doesFileExist(file) then
		print("https://github.com/qrlk/gmap/raw/master/resource/" .. nam)
